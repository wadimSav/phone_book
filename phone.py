...
# Главное окно
    def init_main(self):
        ...
        btn_open_dialog = tk.Button(toolbar, text='Добавить', command=self.open_dialog, bg='beige',
        ...
        btn_refresh = tk.Button(toolbar, text='Все контакты', bg='beige', bd=0, image=self.refresh_img,
...
# Поиск данных
    def search_records(self, find_user):
        find_user = ('%' + find_user + '%','%' + find_user + '%','%' + find_user + '%',)
        self.db.c.execute("SELECT * FROM phonebook WHERE fio LIKE ? OR category LIKE ? OR phone LIKE ?", find_user)
        [self.tree.delete(i) for i in self.tree.get_children()]
        [self.tree.insert('', 'end', values=row) for row in self.db.c.fetchall()]
...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        self.combobox = ttk.Combobox(self, values=[u'Семья', u'Друзья', 'Работа'])
